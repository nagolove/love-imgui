...
function love.load(arg)
    imgui.Init()
    --imgui.SetGlobalFontFromFileTTF('DroidSansMono.ttf', 30)
    --imgui.SetGlobalFontFromFileTTF('comic.ttf', 10)
    --imgui.SetGlobalFontFromArchiveTTF('comic.ttf', 10)
    --imgui.SetGlobalFontFromArchiveTTF('_DroidSansMono.ttf', 130)
...
function love.draw()
    ...
    if showAnotherWindow then
        ...
        imgui.Text("Hello")
        -- Input text
        -- TODO Multiline text not work
        ----textValue = imgui.InputTextMultiline("InputText", textValue, 200, 300, 200)
        textValue = imgui.InputText("InputText", textValue, 200, 300, 200)
        imgui.End()
    end
...
function love.keypressed(key)
    ...
    if not imgui.GetWantCaptureKeyboard() then
        if key == "escape" then
            love.event.quit()
        elseif key == "1" then
            imgui.SetGlobalFontFromArchiveTTF('DroidSansMono.ttf', 30)
            --imgui.SetGlobalFontFromFileTTF('DroidSansMono.ttf', 30)
        elseif key == "2" then
            imgui.SetGlobalFontFromArchiveTTF('comic.ttf', 30)
            --imgui.SetGlobalFontFromFileTTF('comic.ttf', 30)
            imgui.Init()
        end
    end
end
